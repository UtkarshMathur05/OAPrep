...
def test_list_shape(client):
    body = client.get("/api/problems", params={"limit": 3}).json()
...
def test_pagination_differs(client):
    a = client.get("/api/problems", params={"limit": 2, "offset": 0}).json()["problems"]
    b = client.get("/api/problems", params={"limit": 2, "offset": 2}).json()["problems"]
    if len(a) == 2 and len(b) >= 1:
        assert {r["slug"] for r in a}.isdisjoint({r["slug"] for r in b})


def test_difficulty_filter(client):
    rows = client.get("/api/problems", params={"difficulty": "hard", "limit": 5}).json()["problems"]
    assert all(r["difficulty"] == "hard" for r in rows)


def test_company_filter_uses_gin(client):
    body = client.get("/api/problems", params={"company": "google", "limit": 5}).json()
...
def test_search_filter(client):
    ...
    """
    rows = client.get("/api/problems", params={"search": "path", "limit": 5}).json()["problems"]
    for row in rows:
        detail = client.get(f"/api/problems/{row['slug']}").json()
        assert "path" in (detail["title"] + detail["description"]).lower()


def test_topic_filter(client):
    rows = client.get("/api/problems", params={"topic": "Dynamic Programming", "limit": 5}).json()["problems"]
...
def test_sort_is_whitelisted(client):
    ...
    body = client.get("/api/problems", params={"sort": "'; DROP TABLE problems; --"}).json()
    assert body["total"] > 0
...
def test_facets_route_beats_id_route(client):
    ...
    assert client.get("/api/problems/facets").status_code == 200
...
def test_companies_are_ranked_not_alphabetical(client):
    ...
    rows = client.get("/api/problems", params={"limit": 5}).json()["problems"]
...
def test_limit_out_of_range_is_422(client, limit):
    assert client.get("/api/problems", params={"limit": limit}).status_code == 422


def test_detail_by_slug_and_uuid(client, any_slug):
    by_slug = client.get(f"/api/problems/{any_slug}").json()
    by_uuid = client.get(f"/api/problems/{by_slug['id']}").json()
...
def test_detail_404_not_500(client):
    ...
    for ident in ("does-not-exist", "00000000-0000-0000-0000-000000000000", "not-a-uuid"):
        assert client.get(f"/api/problems/{ident}").status_code == 404
