...
def test_mock_mode_returns_shaped_genome(client):
    body = client.post("/api/memory", json={"transcript": "grid, right or down, min cost"}).json()
...
def test_memory_is_persisted_with_real_uuid(client):
    ...
    body = client.post("/api/memory", json={"transcript": "a remembered grid problem"}).json()
...
def test_round_trip_keeps_all_seven_fields(client):
    ...
    created = client.post("/api/memory", json={"transcript": "round trip check"}).json()
    fetched = client.get(f"/api/memory/{created['memory_id']}").json()
...
def test_empty_transcript_is_422(client):
    assert client.post("/api/memory", json={"transcript": "   "}).status_code == 422


def test_unknown_memory_is_404(client):
    for ident in ("00000000-0000-0000-0000-000000000000", "not-a-uuid"):
        assert client.get(f"/api/memory/{ident}").status_code == 404
...
def test_real_extraction_path(client, monkeypatch):
    ...
    body = client.post("/api/memory", json={"transcript": "something about a rotated array"}).json()
    ...
    stored = client.get(f"/api/memory/{body['memory_id']}").json()
...
def test_missing_key_falls_back_to_mock(client, monkeypatch):
    ...
    resp = client.post("/api/memory", json={"transcript": "no key configured"})
